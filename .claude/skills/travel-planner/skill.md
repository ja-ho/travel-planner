...
2. **`<trip-name>` 결정**: `YYYY-MM-<목적지슬러그>` 형식 (예: `2026-04-everland`, `2026-07-tokyo`). 여행일자가 미확정이면 기획 시작 월(YYYY-MM)을 사용한다. 같은 목적지를 여러 번 가는 경우 월 단위로 자연 구분된다.
3. `trips/<trip-name>/` 디렉토리를 프로젝트 루트에 생성한다. 이미 존재하는 경우 사용자에게 덮어쓸지 새 이름을 쓸지 확인한다 (예: `2026-04-everland-2`)
4. 입력을 정리하여 `trips/<trip-name>/00_input.md`에 저장한다
5. 요청 범위에 따라 **실행 모드를 결정**한다

**이전 여행 참고**: `trips/` 아래 다른 폴더는 과거 기획이며, 동일 목적지 재방문이나 패턴 참고용으로 읽기 전용으로 활용한다. 새 여행 산출물을 거기에 덮어쓰지 않는다.

### Phase 2: 팀 구성 및 실행

| 순서 | 작업 | 담당 | 의존 | 산출물 |
|------|------|------|------|--------|
| 1 | 목적지 분석 | analyst | 없음 | `trips/<trip-name>/01_destination_analysis.md` |
| 2 | 일정 설계 + 숙소 | designer | 작업 1 | `trips/<trip-name>/02_itinerary.md`, `trips/<trip-name>/03_accommodation.md` |
| 3a | 예산 계획 | budget | 작업 1, 2 | `trips/<trip-name>/04_budget.md` |
| 3b | 현지 정보 | guide | 작업 1, 2 | `trips/<trip-name>/05_local_guide.md` |
...
1. `trips/<trip-name>/` 내 모든 파일을 확인한다
...
**기존 파일 활용**: 사용자가 기존 일정을 제공하면, `trips/<trip-name>/02_itinerary.md`로 복사하고 designer를 건너뛴다.

## 데이터 전달 프로토콜

| 전략 | 방식 | 용도 |
|------|------|------|
| 파일 기반 | `trips/<trip-name>/` 디렉토리 | 주요 산출물 저장 및 공유 |
...
**기대 결과**:
- 기존 일정을 `trips/<trip-name>/02_itinerary.md`로 복사
